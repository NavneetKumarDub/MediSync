...
exports.generateSlotsForNextDay = exports.startSlotGeneratorJob = void 0;
...
const buildLocalDateStr = (date) => {
    const y = date.getFullYear();
    const m = String(date.getMonth() + 1).padStart(2, '0');
    const d = String(date.getDate()).padStart(2, '0');
    return `${y}-${m}-${d}`;
};
const parseUTCDateFromPg = (rawDate) => {
...
const generateSlotsForNextDay = async () => {
    try {
        const allDoctors = await db_1.default.query(`SELECT DISTINCT user_id FROM doctor_availability WHERE is_active = TRUE`);
        ...
        for (const doctor of allDoctors.rows) {
            ...
            try {
                const lastDateResult = await db_1.default.query(`SELECT MAX(date) as last_date FROM appointment_slots WHERE doctor_id = $1`, [doctorId]);
                let targetDate;
                ...
                const templates = await db_1.default.query(`SELECT
                        user_id,
                        start_time,
                        end_time,
                     ...
                     WHERE user_id = $1
                       AND day_of_week = $2
                       AND is_active = TRUE`, [doctorId, targetDay]);
...
exports.generateSlotsForNextDay = generateSlotsForNextDay;
const startSlotGeneratorJob = () => {
    node_cron_1.default.schedule('17 20 * * *', async () => {
...
exports.startSlotGeneratorJob = startSlotGeneratorJob;
